...
CREATE TABLE IF NOT EXISTS "match_players" (
	...
	"first_turret" integer,
	"takedowns" integer,
	"team_baron_kills" integer,
	"team_rift_herald_kills" integer,
	"team_elder_dragon_kills" integer,
	"team_dragon_kills" integer,
	"percent_team_damage_dealt" integer,
	"percent_team_damage_taken" integer,
	"ward_takedowns_before_20_mins" integer,
	...
	"control_wards_time_coverage" integer,
	"first_dragon_time" integer,
	"early_gold_advantage" integer,
	"early_exp_advantage" integer,
	"kills_near_enemy_turret" integer,
	"kills_under_own_turret" integer,
	"cs_first_10_mins" integer,
	"max_cs_advantage_on_lane_opponent" integer,
	"max_level_lead_on_lane_opponent" integer,
	"kills_while_outnumbered" integer,
	"solo_baron_kills" integer,
	"champion_level_end" integer,
	"gold_spent" integer,
	"gold_earned" integer,
	"perks" json,
	"styles" json,
	"kills_after_hidden_with_ally" integer,
	"skillshots_hit_early_game" integer,
	"turrets_taken_before_10_mins" integer,
	"first_ace" integer,
	"game_ended_surrender" integer,
	"game_ended_early_surrender" integer,
...
--> statement-breakpoint
DO $$ BEGIN
 ALTER TABLE "items_by_champion" ADD CONSTRAINT "items_by_champion_item_id_items_item_id_fk" FOREIGN KEY ("item_id") REFERENCES "public"."items"("item_id") ON DELETE no action ON UPDATE no action;
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
--> statement-breakpoint
...
--> statement-breakpoint
DO $$ BEGIN
 ALTER TABLE "player_items" ADD CONSTRAINT "player_items_item_id_items_item_id_fk" FOREIGN KEY ("item_id") REFERENCES "public"."items"("item_id") ON DELETE no action ON UPDATE no action;
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
